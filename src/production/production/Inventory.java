...
public class Inventory {
   ...
   */
  public Inventory(Floor floor, SimRandom rand)  {
	this.floor = floor;
  ...
  public int stockCount(Item i) {
	return this.stockCount(i.itemNum);
    }
  /**
   * @author Ted Herman
  ...
  public Shelf findItem(Item i) {
	for (Item e: stock) {
	  if (!e.location.onFloor()) continue;  // ignore moving shelves
	...
	// shelf
	Item magic = new Item(i.itemNum,i.description);  // just what we need
  ...
  public Item removeItem(Item a, Shelf s) {
	for (Item e: stock) {
	  if (e.location.onFloor()) continue; // only remove from carried Shelf
      if (!e.equals(a)) continue;   // look for this item only
      if (!e.location.home.equals(s.home)) continue; // only Shelf s
      stock.remove(e);
